...
\begin{document}
    ...
    \begin{example}
        ...
        % \begin{algorithm}[H]
        %     \begin{algorithmic}[1]
        %     \REQUIRE Integers $a$, $b$ with $a = \sum_{i=0}^{n} a_i 10^i$, $b = \sum_{j=0}^{m} b_j 10^j$
        %     \ENSURE $a \cdot b$
        %     \STATE $result \gets 0$
        %     \FOR{$j = 0$ to $m$}
        %         \STATE $partial \gets a \cdot b_j$
        %         \STATE $partial \gets partial \times 10^j$
        %         \STATE $result \gets result + partial$
        %     \ENDFOR
        %     \RETURN $result$
        %     \end{algorithmic}
        % \end{algorithm}
